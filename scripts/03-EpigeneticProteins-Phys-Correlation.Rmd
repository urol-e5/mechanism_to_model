...
correlation_results <- count_phys %>%
  ...
  #filter(estimate > 0) %>%
  mutate(query_id = factor(query_id, levels = query_id[order(estimate)]))
#correlation_results <- correlation_results[c(1:20),]
...
Trying to figure out how to plot this in a way that will make sense...
...
subset_count_phys <- count_phys %>%
  filter(query_id %in% sig_prot) %>%
  full_join(correlation_results, by = "query_id")

ggplot(subset_count_phys, aes(x = PC1, y = value, color = query_id)) + 
  geom_point(size = 3) +
  #geom_curve() +
  facet_wrap(~query_id, scales = "free") +
  theme(legend.position = "none")
...
```{r}
# Create a column to flag significant query_ids
subset_count_phys$significant <- ifelse(subset_count_phys$p.value < 0.05, "Significant", "Not Significant")

# Plot with facets for each query_id, highlighting significance, adding a line, and showing estimate only once
ggplot(subset_count_phys, aes(x = PC1, y = value, color = significant)) +
  ...
  # Add the estimate as a single text label at the top right of each facet
  geom_text(data = subset_count_phys[!duplicated(subset_count_phys$query_id), ], 
            aes(label = paste("Estimate:", round(estimate, 2))), 
            x = Inf, y = Inf, hjust = 1.1, vjust = 1.5, size = 3, inherit.aes = FALSE) +
  
  # Customize the colors for significant vs. not significant
  scale_color_manual(values = c("Significant" = "red", "Not Significant" = "black")) +
  
  # Remove the legend for significance
  theme(legend.position = "none") +
  
  # Add titles and labels
  labs(x = "Energetic State (PC1)", y = "Expression Counts")
```
...
# View the summed outliers by query_id
head(count_phys_outliers)
```

Remove outliers and rerun correlation
```{r}
count_phys_no_outliers <- count_phys_outliers %>%
  filter(is_outlier == FALSE)
